...
route1 =[['10.0.1.0/24', '10.0.1.2', 's1-eth1', '10.0.1.1', [1,2]],
        ['10.0.2.0/24', '10.0.2.2', 's1-eth3', '10.0.2.5', [3]]]

route2 =[['10.0.2.0/24', '10.0.2.2', 's2-eth1', '10.0.2.1', [1]],
        ['10.0.1.0/24', '10.0.2.5', 's2-eth1', '10.0.2.1', [2]]]

gateways1 = ['10.0.1.1','10.0.2.5']
gateways2 = ['10.0.2.1']
...
class MyRouter (object):
  ...
  def _handle_ConnectionUp(self,event):
    ...
    if dpid == 1:
        self.fakeways[dpid]=gateways1
        self.route_table[dpid]=route1
        self.add_ip_flow("10.0.2.0/24", self.mac[2], 3, 1)
        self.fw_block_tcp_web("10.0.1.2","10.0.2.2",dpid)
    elif dpid == 2:
        self.fakeways[dpid]=gateways2
        self.route_table[dpid]=route2
        self.add_ip_flow("10.0.1.0/24", self.mac[1], 2, 2)
...
#    self.set_switch_mac(dpid)
  ...
  def act_like_switch (self, event):
    ...
    log.debug("switch %s: %s port %d", str(dpid), smac, packet_in.in_port)

    if dmac in self.mac_to_port[dpid]:
      # Send packet out the associated port
      out_port=self.mac_to_port[dpid][dmac]
      log.debug("found mac %s port %d", dmac,out_port)
      self.resend_packet(packet_in, out_port, dpid)
    elif dmac != str(self.mac[dpid]):
      #only flood in vlan s1 port 1,2
      if dpid==1 and inport < 3:
        if inport ==1: inport=2 
        else: inport=1
        self.resend_packet(packet_in, inport, dpid)
        log.debug("flood %s -> %s to port %d", smac, dmac, inport)
